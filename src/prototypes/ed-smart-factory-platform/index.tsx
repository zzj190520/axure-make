...
const oeeTrendData = getData('oeeTrendData');
const oeeDetailColumns = getData('oeeDetailColumns');
const oeeDetailData = getData('oeeDetailData');
      ...
      <div className="dsa-table-card">
        ...
        <Table
          ...
          columns={[
            ...
            { title: '设备实时状态', dataIndex: 'status', width: 110, align: 'center', render: (v: string) => {
              const colorMap: Record<string, string> = {
                '运行': statusColors.run,
                '故障': statusColors.fault,
                '空闲': statusColors.idle,
                '停机': statusColors.off,
              };
...
const OEEDetailView = () => {
  const trendRef = useChart(() => ({
    ...
    legend: { data: oeeTrendData.series.map(s => s.name), top: 8, textStyle: { fontSize: 12, color: '#475569' } },
    grid: { top: 50, left: 16, right: 24, bottom: 24, containLabel: true },
    xAxis: { type: 'category', data: oeeTrendData.xAxis, axisLabel: { fontSize: 11, color: '#64748b', rotate: 30 }, axisLine: { lineStyle: { color: '#e2e8f0' } } },
    yAxis: { type: 'value', min: 40, max: 105, axisLabel: { fontSize: 11, color: '#64748b', formatter: '{value}' }, splitLine: { lineStyle: { color: '#f1f5f9' } } },
    series: oeeTrendData.series.map(s => ({
      name: s.name,
      type: 'line',
      data: s.data,
      smooth: true,
      ...
      label: { show: true, position: 'top', fontSize: 10, color: s.color, formatter: '{c}' },
    })),
  }), []);

  const columns = oeeDetailColumns.map(col => {
    if (col.dataIndex === 'category') {
      return {
        ...col,
        onCell: (record: typeof oeeDetailData[0]) => ({ rowSpan: record.categoryRowSpan }),
      };
    }
    if (col.dataIndex === 'name') {
      return {
        ...col,
        onCell: (record: typeof oeeDetailData[0]) => ({ rowSpan: record.nameRowSpan }),
      };
    }
    return col;
  });
  ...
  return (
    <div>
      <div style={{ background: '#fff', borderRadius: 8, padding: '16px 16px 8px', marginBottom: 16, border: '1px solid #e2e8f0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
          <div style={{ width: 4, height: 16, background: '#0ea5e9', borderRadius: 2 }} />
          <span style={{ fontSize: 14, fontWeight: 600, color: '#1e293b' }}>OEE趋势图</span>
        </div>
      ...
      <div style={{ background: '#fff', borderRadius: 8, padding: 16, border: '1px solid #e2e8f0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
          <div style={{ width: 4, height: 16, background: '#0ea5e9', borderRadius: 2 }} />
          <span style={{ fontSize: 14, fontWeight: 600, color: '#1e293b' }}>OEE明细表</span>
        </div>
        <Table
          columns={columns}
          dataSource={oeeDetailData}
          pagination={false}
          size="small"
...
const LineDailyView = () => {
  ...
  const tabItems = [
    ...
    { key: 'oee', label: 'OEE明细', children: <OEEDetailView /> },
